# File copied from https://github.com/beir-cellar/beir/blob/f062f038c4bfd19a8ca942a9910b1e0d218759d4/beir/retrieval/search/lexical/elastic_search.py
import logging
import sys
...
class ElasticSearch(object):
    ...
    def delete_index(self):
        ...
        try:
            ...
            logging.error("Unable to delete Index in Elastic Search. Reason: {}".format(e))
    ...
    def lexical_search(self, text: str, top_hits: int, ids: List[str] = None, skip: int = 0) -> Dict[str, object]:
        ...
        if ids:
            req_body = {"query": {"bool": {"must": req_body["query"], "filter": {"ids": {"values": ids}}}}}

        res = self.es.search(
            search_type="dfs_query_then_fetch", index=self.index_name, body=req_body, size=skip + top_hits
        )
    ...
    def hit_template(self, es_res: Dict[str, object], hits: List[Tuple[str, float]]) -> Dict[str, object]:
        ...
        result = {
            "meta": {"total": es_res["hits"]["total"]["value"], "took": es_res["took"], "num_hits": len(hits)},
            "hits": hits,
        }
        return result
