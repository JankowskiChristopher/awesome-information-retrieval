...
from src.rerankers.rank_llm.rankllm import RankLLM
from src.rerankers.rank_llm.result import LLMRerankerResult, ResultsWriter
...
class Reranker:
    ...
    def rerank(
        self,
        retrieved_results: List[LLMRerankerResult],
        rank_start: int = 0,
        rank_end: int = 100,
        window_size: int = 20,
        step: int = 10,
        ...
        Args:
            retrieved_results (List[LLMRerankerResult]): The list of results to be reranked.
    ...
    def write_rerank_results(
        ...
        results: List[LLMRerankerResult],
        shuffle_candidates: bool = False,
        ...
        Args:
            ...
            results (List[LLMRerankerResult]): The reranked results to be written.
        ...
        Path(f"{rerank_results_dirname}/{retrieval_method_name}/").mkdir(parents=True, exist_ok=True)
        ...
        writer.write_in_json_format(f"{rerank_results_dirname}/{retrieval_method_name}/{name}.json")
        # Write ranking execution summary
        Path(f"{ranking_execution_summary_dirname}/{retrieval_method_name}/").mkdir(parents=True, exist_ok=True)
        writer.write_ranking_exec_summary(f"{ranking_execution_summary_dirname}/{retrieval_method_name}/{name}.json")
