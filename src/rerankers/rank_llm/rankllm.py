...
from src.rerankers.rank_llm.result import LLMRerankerResult, RankingExecInfo
...
class RankLLM(ABC):
    ...
    @abstractmethod
    def create_prompt(
        self, result: LLMRerankerResult, rank_start: int, rank_end: int
    ) -> Tuple[Union[str, List[Dict[str, str]]], int]:
        """
        Abstract method to create a prompt based on the result and given ranking range.

        Args:
            result (LLMRerankerResult): The result object containing data for prompt generation.
    ...
    def permutation_pipeline(
        ...
        Args:
            result (LLMRerankerResult): The result object to process.
        ...
        Returns:
            LLMRerankerResult: The processed result object after applying permutation.
        """
        prompt, in_token_count = self.create_prompt(result, rank_start, rank_end)
        if logging:
            print(f"prompt: {prompt}\n")
        permutation, out_token_count = self.run_llm(prompt, current_window_size=rank_end - rank_start)
        if logging:
    ...
    def sliding_windows(
        self,
        retrieved_result: LLMRerankerResult,
        rank_start: int,
        rank_end: int,
        window_size: int,
        step: int,
        ...
        Args:
            retrieved_result (LLMRerankerResult): The result object to process.
            rank_start (int): The start index for ranking.
            rank_end (int): The end index for ranking.
        ...
        Returns:
            LLMRerankerResult: The result object after applying the sliding window technique.
        """
    ...
    def get_ranking_cost_upperbound(
        ...
        input_token_count = num_q * num_promt * (self._context_size - self.num_output_tokens())
    ...
    def receive_permutation(
        self, result: LLMRerankerResult, permutation: str, rank_start: int, rank_end: int
    ) -> LLMRerankerResult:
        """
        ...
        Args:
            result (LLMRerankerResult): The result object containing the initial ranking results.
        ...
        Returns:
            LLMRerankerResult: The updated result object with the new ranking order applied.
