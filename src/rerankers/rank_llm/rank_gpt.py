...
from src.rerankers.rank_llm.rankllm import PromptMode, RankLLM
...
class SafeOpenai(RankLLM):
    def __init__(
        ...
        if prompt_mode not in [PromptMode.RANK_GPT, PromptMode.LRL]:
            raise ValueError(f"unsupported prompt mode for GPT models: {prompt_mode}, expected RANK_GPT or LRL.")
    ...
    def num_output_tokens(self, current_window_size: Optional[int] = None) -> int:
        ...
        if self._output_token_estimate and self._window_size == current_window_size:
            ...
            _output_token_estimate = (
                len(encoder.encode(" > ".join([f"[{i+1}]" for i in range(current_window_size)]))) - 1
            )
